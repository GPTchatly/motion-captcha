...
export function createRuntimeConfig(env = process.env, port = 3000) {
  ...
  return {
    ...
    maxActiveChallenges: parseIntegerSetting(env, 'MAX_ACTIVE_CHALLENGES', 2_000, 1, 100_000),
    maxActiveSessions: parseIntegerSetting(env, 'MAX_ACTIVE_SESSIONS', 10_000, 1, 500_000),
    maxLimiterKeys: parseIntegerSetting(env, 'MAX_LIMITER_KEYS', 20_000, 10, 1_000_000),
    maxActiveRequests: parseIntegerSetting(env, 'MAX_ACTIVE_REQUESTS', 128, 8, 10_000),
    maxConnections: parseIntegerSetting(env, 'MAX_CONNECTIONS', 256, 8, 100_000),
    challengeBurstLimit: parseIntegerSetting(env, 'CHALLENGE_BURST_LIMIT', 10, 1, 1_000),
    verificationBurstLimit: parseIntegerSetting(env, 'VERIFICATION_BURST_LIMIT', 6, 1, 1_000),
    globalChallengeLimit: parseIntegerSetting(env, 'GLOBAL_CHALLENGE_LIMIT', 200, 1, 100_000),
    globalVerificationLimit: parseIntegerSetting(env, 'GLOBAL_VERIFICATION_LIMIT', 500, 1, 100_000),
    dailyChallengeQuota: parseIntegerSetting(env, 'DAILY_CHALLENGE_QUOTA', 200, 1, 10_000)
  };
}
...
export function resolveClientAddress(request, config) {
  ...
  const forwardedAddresses = forwardedHeader.split(',').map((value) => normalizeIpAddress(value.trim()));
