...
export function createRuntimeConfig(env = process.env, port = 3000) {
  ...
  if (!isProduction && !LOOPBACK_HOSTS.has(publicOriginUrl.hostname)) {
    throw new Error('Non-production PUBLIC_ORIGIN is restricted to loopback hosts.');
  }
  ...
  return {
    ...
    securityContextBurstLimit: parseIntegerSetting(env, 'SECURITY_CONTEXT_BURST_LIMIT', 30, 1, 300),
    accessBurstLimit: parseIntegerSetting(env, 'ACCESS_BURST_LIMIT', 30, 1, 300),
    ...
    globalSecurityContextLimit: parseIntegerSetting(env, 'GLOBAL_SECURITY_CONTEXT_LIMIT', 1_000, 1, 50_000),
    globalAccessLimit: parseIntegerSetting(env, 'GLOBAL_ACCESS_LIMIT', 2_000, 1, 50_000),
    dailyChallengeQuota: parseIntegerSetting(env, 'DAILY_CHALLENGE_QUOTA', 20, 1, 1_000)
  };
}
